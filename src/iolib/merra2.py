...
def rounder(t, interval):
    ...
    if 'inst' in interval:
        ...
        if 'M' in interval:
            t = datetime.strptime(t, '%Y%m%d%H%M%S')
            minusT = (t - relativedelta(months=1)).replace(day=1, second=0, minute=0, hour=0).strftime('%Y%m%d%H%M%S')
            originalT = t.replace(day=1, second=0, minute=0, hour=0).strftime('%Y%m%d%H%M%S')
            plusT = (t + relativedelta(months=1)).replace(day=1, second=0, minute=0, hour=0).strftime('%Y%m%d%H%M%S')
        ...
        if 'M' in interval:
            t = datetime.strptime(t, '%Y%m%d%H%M%S')
            minusT = (t - relativedelta(months=1)).replace(day=1, second=0, minute=30, hour=0).strftime('%Y%m%d%H%M%S')
            originalT = t.replace(day=1, second=0, minute=30, hour=0).strftime('%Y%m%d%H%M%S')
            plusT = (t + relativedelta(months=1)).replace(day=1, second=0, minute=30, hour=0).strftime('%Y%m%d%H%M%S')
...
def merra2_reader(jobID, chunkID):
    """
    ...
    for i, filename in enumerate(files):
        ...
        if fs_s3.exists(filename): # Probably can take this out if the filenames are already validated by s3
            with fs_s3.open(filename, mode='rb') as s3_file_obj:
                ...
                startDate = jobInfo['startDate'].strftime('%Y-%m-%dT%H:%M:%S')
                endDate = jobInfo['endDate'].strftime('%Y-%m-%dT%H:%M:%S')
                ds = ds.sel(time=slice(startDate,endDate), lat=slice(min_lat,max_lat), lon=slice(min_lon,max_lon))
                ...
                if ds['time'].values.size == 0:
                    print('Warning: no times available in this file for the requested range.')
                    continue
                var_data = ds[var]
                if read_count == 0:
...
def merra2_curator(jobID, chunkID):
    """
    ...
    if chunkID == 1 and nChunks > 1:
        if timeStep == 'hourly':
            ...
            newTimestamps = padTimestamps(hierarchyTimes, {'units':'months','quantity':1}, first=True)
        else:
            exit('Cannot figure out chunk scheme')
    elif chunkID == nChunks and nChunks > 1:
        if timeStep == 'hourly':
            newTimestamps = padTimestamps(hierarchyTimes, {'units':'hours','quantity':1}, last=True)
        elif timeStep == 'monthly':
            newTimestamps = padTimestamps(hierarchyTimes, {'units':'months','quantity':1}, last=True)
        else:
            exit('Cannot figure out chunk scheme')
    elif nChunks == 1:
        if timeStep == 'hourly':
            ...
            newTimestamps = padTimestamps(hierarchyTimes, {'units':'months','quantity':1}, first=True, last=True)
        else:
            exit('Cannot figure out chunk scheme')
    else:
        if timeStep == 'hourly':
            ...
            newTimestamps = padTimestamps(hierarchyTimes, {'units':'months','quantity':1})
        else:
            exit('Cannot figure out chunk scheme')
    print(newTimestamps)
    for h in newTimestamps:
        ...
        for thisAnomaly in anomalies:
            ...
            if len(variableData) != 0:
                ...
                data.LongName = variableAttrs['long_name']
...
def merra2_curator_stationary(jobID, chunkID):
    """
    Function to cruate data for MERRA-2, working on multiple datasets.
    This will read data from S3, and subset it to the bounds of the anomaly.  It provides
    data for three time steps (t-1, t, t+1) to make sure there is data for temporal interpolation.
    This is a special use-case implementation of the curtor that can be used to generate long term
    data records for an anomaly at a given time and space (that does not move), but you still want
    curated data over the same spot at different times, perhaps before or after the actual event.
    :param jobID: curation jobID
    :type jobID: int
    :param chunkID: curation chunkID
    :type chunkID: int
    ...
    dataRange = timerange(startDate, endDate, interval)
    for thisTime in dataRange:
        thisTime = thisTime.strftime('%Y%m%d%H%M%S')
        newCredsNeeded = checkReauth(creds)
        if newCredsNeeded == 1:
            creds = s3GetTemporaryCredentials(daac)
        threeTimes = rounder(thisTime, fileTypeStep)
        print(threeTimes)
        #create mask group for netCDF-4 file using t
        maskGroupName = thisTime
        if maskGroupName == lastMaskGroupName:
            groupIncrementor += 1
        else:
            groupIncrementor = 1
        ...
        for thisTime in threeTimes:
            ...
            with fs_s3.open(mFile, mode='rb') as s3_file_obj:
                ...
                min_lon, min_lat, max_lon, max_lat = pushBox(1.25, mp)
                sliceTime = datetime.strptime(thisTime, '%Y%m%d%H%M%S').strftime('%Y-%m-%dT%H:%M:%S.%f000')
                #TODO: add in slice times when it doesn't start at 00:00:00 for monthly
                data = ds.sel(time=slice(sliceTime,sliceTime),lat=slice(min_lat,max_lat), lon=slice(min_lon,max_lon))
                cLat = data.lat
                cLon = data.lon
                cVar = data[variable]
        ...
        if len(variableData) != 0:
            ...
            data.LongName = variableAttrs['long_name']
            data.Units = variableAttrs['units']
            data.FmissingValue = variableAttrs['fmissing_value']
            data.Vmax = variableAttrs['vmax']
            data.Vmin = variableAttrs['vmin']
            data.ValidRange = variableAttrs['valid_range']
