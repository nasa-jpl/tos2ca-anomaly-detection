...
def getFileList(fs_s3, location, jobInfo, stage, lastDayIsOverlap=False):
    ...
    """
    if stage == "curation":
        endDate   = jobInfo['endDate'] + timedelta(days=2)
        startDate = jobInfo['startDate'] - timedelta(days=2)
    else:
...
def gpm_reader(jobID, chunkID):
    ...
    ## might not need enumeration and i here
...
def gpm_curator(jobID, chunkID):
    ...
    if chunkID == 1 and nChunks > 1:
        newTimestamps = padTimestamps(hierarchyTimes, {'units':'minutes','quantity':30}, first=True)
    elif chunkID == nChunks and nChunks > 1:
        newTimestamps = padTimestamps(hierarchyTimes, {'units':'minutes','quantity':30}, last=True)
    elif nChunks == 1:
        newTimestamps = padTimestamps(hierarchyTimes, {'units':'minutes','quantity':30}, first=True, last=True)
    else:
        newTimestamps = padTimestamps(hierarchyTimes, {'units':'minutes','quantity':30})
    print(newTimestamps)
...
def gpm_curator_stationary(jobID, chunkID):
    ...
    dateRange = timerange(startDate, endDate, '30min')
    ...
    for thisTime in dateRange:
        ...
        newCredsNeeded = checkReauth(creds)
        if newCredsNeeded == 1:
            print("Get a new credential: "+datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
            creds = s3GetTemporaryCredentials(daac)
        #Read the mask file
        ...
        #Choose and read the right GPM file
        # create a list of three empty lists
        variableData = []
        gpmFileList = []
        for ind,thisTime in enumerate(threeTimes):
            if dataset == 'GPM_3IMERGHH':
                mTime = datetime.strptime(thisTime, '%Y%m%d%H%M%S').strftime('%Y%m%d-S%H%M%S-E')
            elif dataset == 'GPM_MERGIR':
                mTime = datetime.strptime(thisTime, '%Y%m%d%H%M%S').strftime('%Y%m%d%H')
            ...
            #print(mFile)
            gpmFileList.append(mFile)
            if firsttime == 0 and ind < 2:
                # do not read the file, use saved index indices[1] and indices[2]
                variableData.append(indices[ind+1])
                ...
                if dataset == 'GPM_3IMERGHH':
                    with fs_s3.open(mFile, mode='rb') as s3_file_obj:
                        ...
                        for t, thisTime in enumerate(cTime):
                            for i, thisLon in enumerate(cLon):
                                for j, thisLat in enumerate(cLat):
                                    indices[ind].append([thisLat, thisLon, data[variable].values[t][i][j]])
                            variableData.append(indices[ind])                        
                elif dataset == 'GPM_MERGIR':
                    with fs_s3.open(mFile, mode='rb') as s3_file_obj:
                        ds = xr.open_dataset(s3_file_obj)
                        min_lon, min_lat, max_lon, max_lat = pushBox(0.2, mp)
                        ...
                        for t, thisTime in enumerate(cTime):
                            for i, thisLon in enumerate(cLon):
                                for j, thisLat in enumerate(cLat):
                                    indices[ind].append([thisLat, thisLon, data[variable].values[t][j][i]])
                            variableData.append(indices[ind]) 
        maskGroup.InputFiles = ','.join(gpmFileList)
